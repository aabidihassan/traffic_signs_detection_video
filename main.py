...
def localization(image, min_size_components, similitary_contour_with_circle, model, count, current_sign_type):
    ...
    if sign is not None:
        ...
        sign_type = sign_type if sign_type <= 13 else 13
...
def main(args):
    ...
    while True:
        ...
        if sign_type > 0 and (not current_sign or sign_type != current_sign):
            ...
            position = [count, sign_type if sign_type <= 13 else 13, coordinate[0][0], coordinate[0][1], coordinate[1][0], coordinate[1][1]]
            ...
            if sign_type > 0:
                ...
                position = [count, sign_type if sign_type <= 13 else 13, left, top, right, bottom]
                cv2.rectangle(image, coordinate[0],coordinate[1], (0, 255, 0), 1)
                font = cv2.FONT_HERSHEY_PLAIN
                cv2.putText(image,text,(coordinate[0][0], coordinate[0][1] -15), font, 1,(0,0,255),2,cv2.LINE_4)
            elif current_sign:
                position = [count, sign_type if sign_type <= 13 else 13, tl[0], tl[1], br[0], br[1]]
